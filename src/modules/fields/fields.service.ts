...
export class FieldsService {
  ...
  async findAll(filters?: {
    ...
    if (filters?.sport) {
      query.andWhere('field.sport::text ILike :sport', { sport: `%${filters.sport}%` });
    }
    ...
    if (filters?.priceFrom !== undefined) {
      query.andWhere('field.pricePerHour >= :priceFrom', { priceFrom: filters.priceFrom });
    }
    if (filters?.priceTo !== undefined) {
      query.andWhere('field.pricePerHour <= :priceTo', { priceTo: filters.priceTo });
    }
  
    if (filters?.address) {
      query.andWhere('field.address ILike :city', { city: `%${filters.address}%` });
    }
